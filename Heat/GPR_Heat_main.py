...
import argparse
import sys
from GPR_dataLoader import HeatDataLoader
...
parser.add_argument("--x_num", type=int, default=15, help="Number of x grid points")
parser.add_argument("--t_num", type=int, default=1000, help="Number of t grid points")
...
parser.add_argument("--bc_points", type=int, default=256)
...
parser.add_argument('--range_x', type=float, nargs=2, default=[0, 1])
parser.add_argument('--range_t', type=float, nargs=2, default=[0, 1])
parser.add_argument('--n', type=int, default=1, help=' n is the frequency of the sinusoidal initial conditions')
...
parser.add_argument('--u0_str', type=str, default='sin(pi*x)')  # 对流方程中的初始条件
...
# 创建子目录
sub_dir = f"alpha={args.alpha}/ic_function={safe_u0_str}/x_num_{args.x_num},t_num_{args.t_num}/colnum={(args.x_num-1)*int(args.t_num/args.interval)} {args.x_num-1} {args.t_num} {args.interval}/lr={args.lr}/epochs_{args.epochs}"
...
dataset = HeatDataLoader(args.range_x[0], args.range_t[0], args.range_x[1], args.range_t[1], args.x_num, args.t_num, args.interval, args.alpha, args.ic_points, args.bc_points, device, args.u0_str)

# Processing the test data
x_star = np.linspace(0, 1, args.x_truth_grid)
...
usol = gen_heat_exact_solution(args.x_truth_grid, args.t_truth_grid, args.n, args.alpha, x_upper_bound=args.range_x[1], t_upper_bound=args.range_t[1])
