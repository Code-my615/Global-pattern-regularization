...
class GPR_model:
    def __init__(self, params):
       ...
       # Use optimizers to set optimizer initialization and update functions
        ...
        # 初始化训练数据
        self.X_constrain_col = torch.tensor(params["train_data"]["X_constrain_col"], dtype=torch.float32, device=self.device)
        self.X_constrain_bc = torch.tensor(params["train_data"]["X_constrain_bc"], dtype=torch.float32, device=self.device)
        self.A_matrix = params["train_data"]["A_matrix"]
        self.B_matrix = params["train_data"]["B_matrix"]
        self.f = params["train_data"]["f"]
        self.tao = params["train_data"]["tao"]
        self.h = params["train_data"]["h"]
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            U_constrain_col = self.neural_net(self.X_constrain_col)
            U_constrain_bc = self.neural_net(self.X_constrain_bc)

            result = torch.mm(self.A_matrix, U_constrain_col) - self.tao * self.tao * self.f - torch.mm(self.B_matrix, U_constrain_bc)
            loss_regularization = torch.mean((result) ** 2)
            loss = self.lambda_bc * loss_bc + self.lambda_regularization * loss_regularization
